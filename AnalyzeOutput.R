...
b.model <- factor(rep(1:3, each = 50 * 6 * 11), levels = c("1", "2", "3"), labels = c("b.msst", "b.cuts", "b.tso"))
NA.prop  <- factor(rep(rep(1:2, 3), each = 50 * 3 * 11), levels = c("1", "2"), labels = c("0%", "10%"))
times   <- factor(rep(rep(1:3, 6), each = 50 * 11), levels = c("1", "2", "3"), labels = c("30", "60", "90"))
model   <- factor(rep(rep(1:3, each = 4)[1:11], 50 * 3 * 6), levels = c("1", "2", "3"), labels = c("msst", "cuts", "tso"))
est.method <- factor(rep(c(1,1,2,2,1,1,2,2,1,2,2), 50 * 3 * 6), levels = c("1", "2"), labels = c("ml", "bayes"))
format  <- factor(rep(c(1,2,1,2,1,2,1,2,1,1,2), 50 * 3 * 6), levels = c("1", "2"), labels = c("wide", "long"))
...
parameters.true <- parameters[seq(1, 9918, by = 11 * 50 + 1), ]
parameters.true <- parameters.true[rep(1:18, each = 11 * 50), ]

parameters.est <- parameters[-seq(1, 9918, by = 11 * 50 + 1),]
...
var_coeff.true <- var_coeff[seq(1, 9918, by = 11 * 50 + 1), ]
var_coeff.true <- var_coeff.true[rep(1:18, each = 11 * 50), ]

var_coeff.est <- var_coeff[-seq(1, 9918, by = 11 * 50 + 1),]
